...
def _process_csv(name2prob):
    ...
    for file_name in file_names:
        for name, gender in gen_name_gender_from_csv(os.path.join(_RAW_DATA_ROOT, file_name)):
            prob = CLASS2PROB[gender]
            name2prob[name] = prob
    return name2prob


def _process_dbpedia(name2prob):
...
def _process_us_stats(name2prob, start_year=1940):
    """Process yobxxxx.txt files that list first names and their gender."""
    dir_path = os.path.join(_RAW_DATA_ROOT, 'US-Baby-Name-Stats')
    name2prob_stats = compute_gender_probs(dir_path, start_year)
    for name, prob in name2prob_stats.items():
        name2prob.set_fix_item(name, prob)
    return name2prob


def _process_common_names(name2prob):
    ...
    for file_name, gender in file_name2gender.items():
        name2prob = process_common_names(file_name, gender, name2prob)
...
def main():
    name2prob = Name2Prob()
    name2prob = _process_dbpedia(name2prob)
    name2prob = _process_csv(name2prob)
    name2prob = _process_us_stats(name2prob)
    name2prob = _process_common_names(name2prob)

    with open(_PROCESSED_DATA_PATH, 'wb') as pickle_file:
        pickle.dump(dict(name2prob), pickle_file)  # save as a normal dict object
